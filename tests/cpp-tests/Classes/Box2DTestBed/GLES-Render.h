...
// Cocos renderer for Box2D 3.x's callback-based debug draw API.
class GLESDebugDraw
{
public:
    ...
    void setDrawNode(cocos2d::DrawNode* drawNode) { _drawNode = drawNode; }
...
private:
    ...
    cocos2d::Color4F makeColor(b2HexColor color, float alpha = 1.0f, float scale = 1.0f) const;
    cocos2d::Vec2 makePoint(b2Vec2 point) const;
